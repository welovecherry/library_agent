...
# """
# Harvest basic node: navigate catalog index, search, then SAVE LOCALLY (no agent-side write_file).
...
# def run_harvest(place: str, title: str, watchdog_sec: int = 300) -> None:
#     """
...
#     logger = get_logger(f"harvest_{session_id}")
#     logger.info(f"[SESSION {session_id}] run_harvest start | place={place} title={title!r} watchdog={watchdog_sec}s")
...
#         date_dir = os.path.join(_PKG_ROOT, "data", "raw", str(datetime.date.today()))
#         os.makedirs(date_dir, exist_ok=True)
#         safe_base = f"{place}_{int(time.time())}"  # ✅ NEW: 파일명 ASCII-safe

#         # NEW: 에이전트는 파일을 쓰지 않는다. 최종 경로만 준비해서 파이썬이 저장한다.
#         html_path = os.path.join(date_dir, f"{safe_base}.html")
#         text_path = os.path.join(date_dir, f"{safe_base}.txt")
#         logger.info(f"output_html={html_path} output_text={text_path}")
...
#         text_val = None
#         try:
#             # history.action_results() 에서 evaluate 결과만 추출
#             results = getattr(history, "action_results", None)
#             if callable(results):
#                 ars = results()
#             else:
#                 ars = []

#             # evaluate 결과를 뒤에서부터 스캔: 가장 최근 2개를 HTML/TEXT로 간주
#             eval_payloads = []
#             for ar in ars:
#                 # 안전하게 dict-like 접근
#                 tool = getattr(ar, "tool", None) or getattr(ar, "tool_name", None) or ""
#                 res  = getattr(ar, "result", None) or getattr(ar, "response", None)
#                 if isinstance(tool, str) and "evaluate" in tool.lower() and isinstance(res, str):
#                     eval_payloads.append(res)

#             # 뒤에서부터 수집
#             for val in reversed(eval_payloads):
#                 if html_val is None and isinstance(val, str) and val.lstrip().lower().startswith("<html"):
#                     html_val = val
#                 elif text_val is None and isinstance(val, str) and not val.lstrip().lower().startswith("<html"):
#                     text_val = val
#                 if html_val and text_val:
#                     break

#             # 폴백: 개수 부족 시 전체 evaluate 가장 최근 것을 TEXT로라도 저장
#             if html_val is None and eval_payloads:
#                 cand = next((v for v in reversed(eval_payloads) if isinstance(v, str) and v.lstrip().lower().startswith("<html")), None)
#                 html_val = cand
#             if text_val is None and eval_payloads:
#                 cand = next((v for v in reversed(eval_payloads) if isinstance(v, str) and not v.lstrip().lower().startswith("<html")), None)
#                 text_val = cand

#         except Exception as e:
#             logger.exception(f"history parsing failed: {e}")
...
#             else:
#                 logger.warning("no HTML evaluate result found")
#         except Exception:
...
#             else:
#                 logger.warning("no TEXT evaluate result found")
#         except Exception:
...
#     logger.info(f"[SESSION {session_id}] run_harvest end")


# if __name__ == "__main__":
#     # 기본 실행: 강남 / 숨결이 바람 될 때
#     run_harvest("gangnam", "숨결이 바람 될 때", watchdog_sec=300)
...
"""
    ...
    with step(logger, "collect_and_save"):
        # 기존 방식: action_results()에서 evaluate만 집기
        # results = getattr(history, "action_results", None)
        # if callable(results): ars = results() else: ars = []
        # eval_payloads = [...]
        # (위 코드는 저장 실패 원인이었으므로 주석 처리)
        # ----------------------------------------------------------
        # ✅ NEW: 여러 소스에서 문자열 전수 수집 후 HTML/TEXT 선택
        candidates: List[str] = []
